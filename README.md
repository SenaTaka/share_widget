...
1. **Pin a note**: In the note list, swipe left on a note and tap the pin icon
2. **Add widget to home screen**: Long-press on your home screen → tap the "+" button → search for "share_widget" → select widget size → tap "Add Widget"
...
1. **ノートを固定**: ノート一覧でノートを左スワイプし、ピンアイコンをタップ
2. **ウィジェットを追加**: ホーム画面を長押し → 「+」ボタンをタップ → "share_widget"を検索 → ウィジェットサイズを選択 → 「ウィジェットを追加」
